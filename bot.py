...
def save_profile(uid, api_key, base_url):
    conn = get_db()
    cur = conn.cursor()
    cur.execute("INSERT INTO profiles (user_id, api_key, base_url) VALUES (%s,%s,%s) ON CONFLICT (user_id) DO UPDATE SET api_key=%s, base_url=%s",
...
async def start(msg: types.Message):
    ...
    txt += f'<{e("settings")}></tg-emoji> Модель: {name}\n'
    if profile:
        txt += f'<{e("link")}></tg-emoji> API: {profile["base_url"][:30]}...\n'
    else:
        txt += f'<{e("info")}></tg-emoji> Настройте API в Профиле\n'
    txt += f'<{e("info")}></tg-emoji> Отправьте сообщение!'
...
async def msg_api_key(msg: types.Message, state: FSMContext):
    ...
    save_profile(msg.from_user.id, api_key, base_url)
    await state.clear()
    await msg.answer(f'<{e("check")}></tg-emoji> API ключ сохранён!', parse_mode=ParseMode.HTML, reply_markup=main_kb())
...
async def msg_base_url(msg: types.Message, state: FSMContext):
    ...
    save_profile(msg.from_user.id, api_key, base_url)
    await state.clear()
    await msg.answer(f'<{e("check")}></tg-emoji> Base URL сохранён!', parse_mode=ParseMode.HTML, reply_markup=main_kb())
...
async def cb_models(cb: types.CallbackQuery):
    ...
    await cb.message.edit_text(
        f'<{e("settings")}></tg-emoji> <b>Модели</b> ({working} из {len(available_models)} работают):',
...
async def cb_page(cb: types.CallbackQuery):
    ...
    await cb.message.edit_text(f'<{e("settings")}></tg-emoji> <b>Модели</b>:', parse_mode=ParseMode.HTML, reply_markup=model_kb(page))
...
async def cb_new(cb: types.CallbackQuery):
    ...
    await cb.message.edit_text(f'<{e("check")}></tg-emoji> Новый чат!\n<{e("settings")}></tg-emoji> {name}', parse_mode=ParseMode.HTML, reply_markup=main_kb())
...
async def cb_switch(cb: types.CallbackQuery):
    ...
    await cb.message.edit_text(f'<{e("smile")}></tg-emoji> {t}\n<{e("info")}></tg-emoji> Отправьте сообщение!', parse_mode=ParseMode.HTML, reply_markup=main_kb())
...
async def cb_clear(cb: types.CallbackQuery):
    ...
    await cb.message.edit_text(f'<{e("trash")}></tg-emoji> Все чаты удалены!', parse_mode=ParseMode.HTML, reply_markup=main_kb())
