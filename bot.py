...
available_models = {}
user_states = {}
# Храним профили в оперативке ТОЧНО
user_profiles = {}
...
def init_db():
    ...
    cur.execute("""CREATE TABLE profiles (
        user_id BIGINT PRIMARY KEY, api_key TEXT NOT NULL, base_url TEXT NOT NULL)""")
    conn.commit()
    conn.close()
    print("✅ DB recreated")
...
def get_profile(uid):
    ...
    try:
        ...
        cur.execute("SELECT api_key, base_url FROM profiles WHERE user_id=%s", (uid,))
        row = cur.fetchone()
        conn.close()
        if row and row.get("api_key") and row.get("base_url"):
            user_profiles[uid] = {"api_key": row["api_key"], "base_url": row["base_url"]}
            return user_profiles[uid]
    except Exception as ex:
        print(f"❌ get_profile DB error: {ex}")
    return None
...
def save_profile(uid, api_key, base_url):
    try:
        ...
        cur.execute("INSERT INTO profiles (user_id, api_key, base_url) VALUES (%s,%s,%s)", (uid, api_key, base_url))
        conn.commit()
        conn.close()
        # Сохраняем в оперативку
        user_profiles[uid] = {"api_key": api_key, "base_url": base_url}
    ...
    try:
        async with aiohttp.ClientSession() as s:
            async with s.post(url, headers=headers, json=payload, ssl=False, timeout=aiohttp.ClientTimeout(total=15)) as r:
                ...
                if r.status == 200:
                    return True, None
                return False, f"Ошибка {r.status}: {raw[:100]}"
    except Exception as ex:
                ...
                if r.status == 200:
                    ...
                    for mid in sorted(set(models_list)):
                        if mid and not mid.startswith('#'):
                            available_models[mid] = {"name": mid.replace("-", " ").title()[:30], "works": None}
                    
                    for mid in list(available_models.keys())[:8]:
...
async def call_api(model, messages, api_key, base_url):
    ...
    async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=120)) as s:
        async with s.post(url, headers=headers, json=payload, ssl=False) as r:
            ...
            raise Exception(f"Ошибка API: {raw[:300]}")

def model_kb(page=0):
    if not available_models:
        kb = InlineKeyboardBuilder()
        kb.add(InlineKeyboardButton(text="Нет моделей", callback_data="none", icon_custom_emoji_id=EM["info"]))
    ...
    for mid, data in page_models:
        ...
        kb.add(InlineKeyboardButton(text=data.get("name", mid)[:25], callback_data=f"use_{mid}", icon_custom_emoji_id=icon))
    ...
    kb.add(InlineKeyboardButton(text="Проверить все", callback_data="refresh_models", icon_custom_emoji_id=EM["loading"]))
    kb.add(InlineKeyboardButton(text="Назад", callback_data="back_main", icon_custom_emoji_id=EM["back"]))
    kb.adjust(1)
    return kb.as_markup()
...
async def start(msg: types.Message):
    ...
    if profile:
        txt += f'{e("check")}✅</tg-emoji> API настроен\n'
    else:
        txt += f'{e("cross")}❌</tg-emoji> Настройте API в Профиле\n'
...
async def cmd_profile(msg: types.Message):
    ...
    if profile:
        ...
        txt += f'{e("key")}🔑</tg-emoji> Ключ: <code>...{profile["api_key"][-8:]}</code>\n'
        txt += f'{e("link")}🔗</tg-emoji> URL: <code>{profile["base_url"]}</code>'
    else:
...
async def cb_set_key(cb: types.CallbackQuery, state: FSMContext):
    ...
    profile = get_profile(cb.from_user.id)
    current = f"...{profile['api_key'][-8:]}" if profile else "не задан"
    await cb.message.edit_text(
        f'{e("key")}🔑</tg-emoji> <b>Отправьте API ключ:</b>\n\nПример: <code>sk-xxxxxxxxxxxxx</code>\n\nТекущий: <code>{current}</code>',
...
async def cb_set_url(cb: types.CallbackQuery, state: FSMContext):
    ...
    profile = get_profile(cb.from_user.id)
    current = profile["base_url"] if profile else "не задан"
    await cb.message.edit_text(
...
async def msg_api_key(msg: types.Message, state: FSMContext):
    ...
    # Получаем текущий URL или дефолтный
    profile = get_profile(uid)
    base_url = profile["base_url"] if profile else "https://gpt-agent.cc/v1"
    
    ok = save_profile(uid, api_key, base_url)
    await state.clear()
    
    if ok:
        # Проверяем сразу
        saved = get_profile(uid)
        if saved:
            await msg.answer(
                f'{e("check")}✅</tg-emoji> <b>Сохранено!</b>\n\nКлюч: <code>...{saved["api_key"][-8:]}</code>\nURL: <code>{saved["base_url"]}</code>',
                parse_mode=ParseMode.HTML, reply_markup=main_kb()
            )
        else:
            await msg.answer(f'{e("cross")}❌</tg-emoji> Ошибка проверки. /profile', parse_mode=ParseMode.HTML, reply_markup=main_kb())
    else:
        await msg.answer(f'{e("cross")}❌</tg-emoji> Ошибка сохранения!', parse_mode=ParseMode.HTML, reply_markup=main_kb())
...
async def msg_base_url(msg: types.Message, state: FSMContext):
    ...
    profile = get_profile(uid)
    if not profile:
        await msg.answer(f'{e("cross")}❌</tg-emoji> Сначала добавьте API ключ!', parse_mode=ParseMode.HTML, reply_markup=profile_kb())
        await state.clear()
        return
    
    ok = save_profile(uid, profile["api_key"], base_url)
    await state.clear()
    ...
    if ok:
        ...
        if saved:
            ...
            await msg.answer(f'{e("cross")}❌</tg-emoji> Ошибка проверки', parse_mode=ParseMode.HTML, reply_markup=main_kb())
    else:
        await msg.answer(f'{e("cross")}❌</tg-emoji> Ошибка сохранения!', parse_mode=ParseMode.HTML, reply_markup=main_kb())
...
async def cb_models(cb: types.CallbackQuery):
    ...
    if not available_models:
        ...
        await fetch_models(profile["api_key"], profile["base_url"])
    w = sum(1 for d in available_models.values() if d.get("works") is True)
    await cb.message.edit_text(
        f'{e("settings")}⚙️</tg-emoji> <b>Модели</b> ({w} из {len(available_models)} работает):',
...
async def cb_refresh(cb: types.CallbackQuery):
    ...
    w = sum(1 for d in available_models.values() if d.get("works") is True)
    await cb.message.edit_text(
        f'{e("check")}✅</tg-emoji> {w} рабочих из {len(available_models)}',
        parse_mode=ParseMode.HTML, reply_markup=model_kb(0)
    )

@dp.callback_query(F.data.startswith("mp_"))
async def cb_page(cb: types.CallbackQuery):
    await cb.message.edit_text(f'{e("settings")}⚙️</tg-emoji> <b>Модели</b>:', parse_mode=ParseMode.HTML, reply_markup=model_kb(int(cb.data.split("_")[1])))
...
async def cb_use(cb: types.CallbackQuery):
    ...
    await cb.message.edit_text(f'{e("check")}✅</tg-emoji> <b>{available_models.get(mid,{}).get("name",mid)}</b>', parse_mode=ParseMode.HTML, reply_markup=main_kb())

@dp.callback_query(F.data == "new_chat")
async def cb_new(cb: types.CallbackQuery):
    s = get_state(cb.from_user.id)
    s['chat'] = str(uuid.uuid4())
    await cb.message.edit_text(f'{e("check")}✅</tg-emoji> Новый чат!', parse_mode=ParseMode.HTML, reply_markup=main_kb())

@dp.callback_query(F.data == "my_chats")
async def cb_chats(cb: types.CallbackQuery):
    conn = get_db()
    cur = conn.cursor(cursor_factory=RealDictCursor)
    cur.execute("SELECT chat_id, MAX(title) as title, COUNT(*) as cnt FROM chats WHERE user_id=%s GROUP BY chat_id ORDER BY MAX(timestamp) DESC LIMIT 10", (cb.from_user.id,))
    chats = cur.fetchall()
    conn.close()
    kb = InlineKeyboardBuilder()
    if chats:
        for c in chats:
            kb.add(InlineKeyboardButton(text=f"{(c['title'] or 'Новый чат')[:25]} ({c['cnt']})", callback_data=f"switch_{c['chat_id']}", icon_custom_emoji_id=EM["smile"]))
    ...
    kb.add(InlineKeyboardButton(text="Назад", callback_data="back_main", icon_custom_emoji_id=EM["back"]))
    kb.adjust(1)
    await cb.message.edit_text(f'{e("file")}📁</tg-emoji> <b>Ваши чаты:</b>', parse_mode=ParseMode.HTML, reply_markup=kb.as_markup())
