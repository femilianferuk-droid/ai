...
async def msg_api_key(msg: types.Message, state: FSMContext):
    ...
    if ok:
        ...
        saved = get_profile(msg.from_user.id)
        if saved:
            await msg.answer(
            ...
            await msg.answer(
                f'{e("cross")}❌</tg-emoji> Не удалось проверить сохранение. Попробуйте /profile',
                parse_mode=ParseMode.HTML, reply_markup=main_kb()
            )
    else:
        await msg.answer(
...
async def msg_base_url(msg: types.Message, state: FSMContext):
    ...
    if not api_key:
        await msg.answer(
            f'{e("cross")}❌</tg-emoji> Сначала добавьте API ключ!',
            parse_mode=ParseMode.HTML, reply_markup=profile_kb()
        )
        await state.clear()
        return
    ...
    if ok:
        ...
        if saved:
            ...
            await msg.answer(
                f'{e("cross")}❌</tg-emoji> Не удалось проверить сохранение.',
                parse_mode=ParseMode.HTML, reply_markup=main_kb()
            )
    else:
        await msg.answer(
            f'{e("cross")}❌</tg-emoji> Ошибка сохранения!',
            parse_mode=ParseMode.HTML, reply_markup=main_kb()
        )
