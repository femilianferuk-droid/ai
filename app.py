...
def get_available_models():
    """Получает список доступных моделей через API с кэшированием"""
    now = datetime.utcnow()
    
    if (_models_cache['data'] and _models_cache['timestamp'] and 
        (now - _models_cache['timestamp']).seconds < Config.MODELS_CACHE_TTL):
    ...
    try:
        ...
        # Возвращаем кэш если он есть, иначе запасной список
...
def init_database():
    """Инициализация базы данных и создание пользователя по умолчанию"""
    try:
        db.create_all()
        ...
        if not db.session.execute(
            ...
            db.session.add(user)
            db.session.commit()
            print(f"✓ User '{Config.DEFAULT_USERNAME}' created")
