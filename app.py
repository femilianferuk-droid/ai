...
openai.api_key = "sk-txA1lHYWAWWKKSMnfjkZNo2gRgvjfUtKq7PZWgkA0WMDIxOB"
openai.api_base = "https://gpt-agent.cc/v1"
...
def call_api(model, messages):
    try:
        resp = openai.ChatCompletion.create(model=model, messages=messages, temperature=0.7, max_tokens=2000)
        return resp.choices[0].message.content
    except Exception as e:
        err = str(e)
        # Проверяем ошибку лимита
        if '额度不足' in err or '额度' in err:
            raise Exception("💰 Закончился баланс API. Пополните счёт на gpt-agent.cc")
        # Ретрай без system message
        msgs = [m for m in messages if m["role"] != "system"]
        resp = openai.ChatCompletion.create(model=model, messages=msgs, temperature=0.5, max_tokens=1000)
        return resp.choices[0].message.content
...
async def handle(msg: types.Message):
    ...
    try:
        ans = await asyncio.to_thread(call_api, model, msgs)
        s['busy'] = False
        tsk.cancel()
        ...
        tsk.cancel()
        err = str(ex)
        if 'баланс' in err.lower() or 'пополнит' in err.lower():
            await st.edit_text(f'{e("money")}💰</tg-emoji> {err}', parse_mode=ParseMode.HTML)
        else:
            await st.edit_text(f'{e("cross")}❌</tg-emoji> {err[:500]}', parse_mode=ParseMode.HTML)
