...
def send_message():
    try:
        ...
        cur.execute(
            ...
            (chat_id, title, model, 'user', user_message)
        )
        
        # Обновляем заголовок для первого сообщения
        if msg_count == 0:
            cur.execute(
                "UPDATE chats SET title = %s WHERE chat_id = %s AND title = 'Новый чат'",
        ...
        async function sendMessage() {
            ...
            try {
                ...
                if (data.response) {
                    ...
                    addMessageToUI('assistant', `❌ Ошибка: ${data.error || 'Неизвестная ошибка'}`, currentModel, new Date().toISOString());
                }
            } catch (error) {
                removeTypingIndicator();
