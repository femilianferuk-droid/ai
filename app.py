...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; background: #0f0f1a; color: #e0e0e0; display: flex; align-items: center; justify-content: center; height: 100vh; }
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  :root { --bg: #0f0f1a; --sidebar: #13131f; --border: #1e1e30; --accent: #7c3aed; --hover: #1a1a2e; --text: #e0e0e0; --muted: #6b6b8a; --user-msg: #2d1f7a; --ai-msg: #1a1a2e; }
  ...
  .session-item.active { background: var(--hover); color: var(--text); border-left: 3px solid var(--accent); }
  .session-item span { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; flex: 1; }
  .delete-btn { background: none; border: none; color: #555; cursor: pointer; font-size: 16px; padding: 0 4px; opacity: 0; transition: opacity 0.2s; }
  ...
  .msg.ai { background: var(--ai-msg); border: 1px solid var(--border); align-self: flex-start; border-bottom-left-radius: 4px; }
  .msg.ai code { background: #0d0d1a; padding: 2px 6px; border-radius: 4px; font-family: 'Fira Code', monospace; font-size: 13px; }
  ...
  .typing-indicator { display: flex; gap: 4px; padding: 14px 18px; border-radius: 12px; background: var(--ai-msg); border: 1px solid var(--border); align-self: flex-start; max-width: 720px; width: 100%; }
  ...
  @media (max-width: 640px) {
    ...
    .sidebar.open { width: 260px; display: flex; }
  }
</style>
</head>
<body>
...
<div class="sidebar" id="sidebar">
  <div class="sidebar-header">
    <h2>🔮 AI Chat</h2>
    <span style="color:#555;font-size:12px;">{{ username }}</span>
  </div>
  <a href="/chat/new" class="new-chat-btn">+ New chat</a>
  <div class="sessions-list">
    {% for s in sessions %}
    <div class="session-item {% if s.id == current_session_id %}active{% endif %}">
      <a href="/chat/{{ s.id }}" style="flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:inherit;text-decoration:none;">
        {{ s.title }}
      </a>
...
<div class="main">
  <div class="chat-header">
    ...
    <span style="font-size:12px;color:#444;">{{ username }}</span>
  </div>
